...
class LessonItemOrderCell: UITableViewCell {
  var subjectType: TKMSubject_Type! {
    willSet(subjectType) {
      textLabel!.text = TKMSubjectTypeName(subjectType)
    }
  }
...
class LessonItemOrderViewController: UITableViewController {
  var inOrderCount: Int {
    Settings.lessonOrder.filter { $0 != TKMSubject_Type.unknown.rawValue }.count
  }

  var itemOrder: [Int32] {
    // Correct any error in the actual settings array.
    var itemOrder = Settings.lessonOrder.filter { $0 != TKMSubject_Type.unknown.rawValue }
    for _ in itemOrder.count ..< Settings.lessonOrder.count {
      itemOrder.append(TKMSubject_Type.unknown.rawValue)
    }
    Settings.lessonOrder = itemOrder

    // Compute the actual order array containing all real subject types.
    var order = Settings.lessonOrder.filter { $0 != TKMSubject_Type.unknown.rawValue }
